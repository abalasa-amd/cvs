...
Usage:
    pytest tests/benchmark/test_aorta.py \
...
# Add lib and root to path for imports
sys.path.insert(0, str(Path(__file__).parent.parent.parent))
sys.path.insert(0, str(Path(__file__).parent.parent.parent / 'lib'))

from runners.aorta import AortaRunner, AortaConfig, AortaDockerConfig, RcclConfig, AortaEnvironment
from runners._base_runner import RunStatus
...
import globals
from utils_lib import fail_test, update_test_result
...
def validated_cluster_config(cluster_file) -> ClusterConfigFile:
    ...
    try:
        config = validate_config_file(cluster_file, config_type="cluster")
...
def aorta_runner_config(
    validated_cluster_config: ClusterConfigFile,
...
class TestAortaBenchmark:
    ...
    def test_parse_results(self, aorta_runner_config, validated_aorta_config):
        ...
        parser = TraceLensParser(use_tracelens=True)  # Use TraceLens if available
        
        # Parse trace files
        parse_result = parser.parse(TestAortaBenchmark.run_result)
    ...
    def test_validate_thresholds(self, validated_aorta_config):
        ...
        failures = parser.validate_thresholds(
            TestAortaBenchmark.benchmark_result,
            expected
        )
